...
; This code is designed to batch resize all images for one flightbox (such as Sierra Nevada or Santa Barbara).
; This code uses a base image (one that has been cropped to study area of choice) for each flight line (such as flightline 1 - 11)
; and adds 10 blank lines to all sides of the image, and convert all images to BSQ.
; All images will be cropped to the same area as the base image. All output images will have the same number of samples and lines, which 
; means the image sizes will be exactly the same.  
; This code outputs images that will be inputs for Alex Koltunov's Image to Image Registration code.
; Susan Meerdink
; Created 4/25/2016

;;; INPUTS ;;;
main_path = 'E:\swshivers\SodaStrawFlightlines\' ; Set directory that holds all flightlines
base_image_id ='f140407t01p00r18_rdn_hpc18_v1_ag' ;search term to identify base image (and base image only!) 
all_image_id = 'f1*' ;search term which needs to apply to all images in the file path you want co-registered
...
fl_list[0,0] = STRCOMPRESS("toCrop",/REMOVE_all) ;Add flightline name to list
...
FOREACH single_flightline, fl_list DO BEGIN ;;; LOOP THROUGH FLIGHTLINES ;;;
  ...
  FOREACH single_image, image_list DO BEGIN ; Loop through all images for a single flightline
    IF strmatch(single_image,'*.hdr') EQ 0 THEN BEGIN ;If the file being processed isn't a header file proceed
      ...
      fileOutput = raster_file_name + '_agForAcorn' ;Set file name for new image
      ...
      ENVI_WRITE_ENVI_FILE, outImage, $ ; Data to write to file
        ...
        R_FID = fidFinal, $ ;Set keyword for new file's FID
      ...
      ;;; CONVERT TO BSQ ;;;
      ENVI_FILE_QUERY,fidFinal, DIMS = new_dims, NS = new_samples, NL = new_lines, NB = new_bands
